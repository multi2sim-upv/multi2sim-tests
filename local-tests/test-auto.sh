...
#M2S_BUILD_PATH=/home/sisco/workspace/multi2sim
#M2S_TEST_PATH=$M2S_CLIENT_KIT_TEST_PATH
#M2S=$M2S_BUILD_PATH/bin/m2s
#M2C=$M2S_BUILD_PATH/bin/m2c
...
# Options
temp=`getopt -o r:h -l dir:, -l tag:,help -n $prog_name -- "$@"`
if [ $? != 0 ] ; then exit 1 ; fi
eval set -- "$temp"
rev=
rev_arg=
...
while true ; do
	...
	--dir) local_dir=$2 ; dir_arg="--dir $2" ; shift 2 ;;
	--) shift ; break ;;
...
# Obtain local copy
$build_m2s_local_sh $rev_arg $tag_arg $dir_arg || exit 1
