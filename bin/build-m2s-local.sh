...
# Options
temp=`getopt -o r:h -l configure-args:,tag:,dir:,help -n $prog_name -- "$@"`
if [ $? != 0 ] ; then exit 1 ; fi
eval set -- "$temp"
rev=
configure_args=
...
then
	...
	echo -n " - linking to new source path"
	ln -s $local_dir $M2S_CLIENT_KIT_M2S_SRC_PATH
	check_m2s_build_result=1
fi
...
#echo "write Build From Directory $local_dir" >> $inifile_script
